...
class VanDerPolDataset(eqx.Module):
    ...
    def __call__(self, key: Array) -> Tuple[Array, Array, Array, Array, Array, Array, Array]:
        ...
        mu = random.uniform(mu_key, (), minval=self.mu_range[0], maxval=self.mu_range[1])
        
        # Generate random initial conditions
        _y0 = random.uniform(
            y0_key, 
            (total_points, 2), 
            minval=self.y0_range[0], 
            maxval=self.y0_range[1]
        )
        ...
        _dt = random.uniform(
            dt_key,
            (total_points,),
            minval=self.dt_range[0],
            maxval=self.dt_range[1]
        )
